...
define("index", ["ZrlStrings", "jquery", "md5", "qrcode", "bootstrap"], function (ZrlStrings, $) {
    var host = null;
    ...
    function getResultPanel(url) {
        ...
        if (!encodeResultPanel) {
            var outputControl = $('<input class="form-control shorten-result"  data-toggle="tooltip" data-placement="bottom" title="' + ZrlStrings.CopyTooltip + '"  readonly="readonly" type="text" value="" tabindex="2">')
                .click(function () {
                    $(this).focus();
                    $(this).select();
                })
            ...
            var p = $('<div style="display:none;" class="well"/>')
                .append(
                    $('<div class="row">').append(
                         $('<div class="col-lg-8"/>')
                        .append($('<div class="input-group"/>').append(outputControl))
                    )
                    .append($('<div class="col-lg-4"/>').append(qrpicture))
                )
                .append('<hr />')
                .append($('<div class="row"/>')
                        .append($('<div class="col-lg-10"/>')
                                .append(sourceBlock)
                         )
                         .append($('<div class="col-lg-2"></div>').append(
                                 $('<button class="btn btn-default">' + ZrlStrings.BtnClose + '</button>')
                                 .click(function () {
                                     p.fadeOut();
                                 })
                                ))
        ...
        if ($.isPlainObject(data)) {
            ...
            if (data.result == 'true') {
                ...
                if (qrinput.val() != result) {
                    ...
                    qrinput.val(result);
                    sourceBlock.text(ZrlStrings.SourceUrl + data.source)
                ...
                showMessage(ZrlStrings['EncodeErrorCode' + data.errorcode]);
            }
        }
        else {
    ...
    function getValidatePanel() {
        ...
        if (!validatePanel) {
            ...
            var verify = $('<button class="btn btn-default disabled" type="button">' + ZrlStrings.BtnVerify + '</button>');

            var verifyInputvalue = $('<input type="text"  autocomplete="off" class="form-control" placeholder="' + ZrlStrings.InputVerifyCodePlaceHolder + '" maxlength="4">');
            var img = $('<img title="' + ZrlStrings.RefreshImgeTooltip + '" class="verify-img" src="' + indexConfig.VerifyCodeUrl + '"/>')
                      .click(function () {
                          verifyInputvalue.val("");
                          refreshImage.apply(this);
                      });
            var imgcontainer = $(' <div class="verify-container image-loading "></div>').append(img);

            var panel = $(' <div id="verifyPanel" style="display:none;" class="well"></div>').append(
                    $('<div></div>')
                        .append(imgcontainer)
                        .append(ZrlStrings.RefreshImgeTooltip)
                );
            ...
            verify.click(function () {
                ...
                $.post(indexConfig.EncodeUrl, data, function (resultObj) {
            ...
            var cancel = $('<button class="btn btn-default" type="button">' + ZrlStrings.BtnCancel + '</button>').click(function () {
                panel.fadeOut();
                enableInput(true);
            });

            panel.append($('<div class="input-group"/>')
                .append(verifyInputvalue)
                .append(
    ...
    function validateUrl(url) {
        ...
        var rg = new RegExp("^(https?)://([a-zA-Z0-9]+\.)?zrl\.so/([0-9a-zA-Z]+$)", "i");
        if (rg.test(url)) {
    ...
    function iniInput() {
        ...
        $("#sourceUrl").tooltip({ title: ZrlStrings.InputUrlHint, placement: 'top' });
    }
